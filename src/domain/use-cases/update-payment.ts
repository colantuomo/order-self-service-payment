...
import { SendFailedPaymentStatus, SendSucessPaymentStatus } from "../../infraestructure/adapters/events";

export async function updatePaymentByExternalIdUseCase({ id }: IUpdatePaymentByExternalIdCommand) {
    try {
        ...
        if (paymentStatus === PaymentStatus.CANCELED) {
            SendFailedPaymentStatus({ id: response.id, orderId: response.orderId });
        }

        if (paymentStatus === PaymentStatus.PAID) {
            SendSucessPaymentStatus({ id: response.id, orderId: response.orderId });
        }
